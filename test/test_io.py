...
from mbr_nmt.io import read_samples_file, EOS_TOKEN

class TestIO(unittest.TestCase):

    def test_read_candidates(self):
        # Test that things are properly tokenized.
        samples = read_samples_file("test/data/samples-4.txt", 4)
        ...
        try:
            samples = read_samples_file("test/data/samples-4.txt", 5)
        except:
            raised_exception = True
        self.assertTrue(raised_exception)

        # Test with a file containing empty sequences.
        samples = read_samples_file("test/data/samples-empty-3.txt", 3)
        expected = [["George went to school by bike today .".split(" "),
                    [],
                    "This is a completely unrelated sentence .".split(" ")],
                    ["He got the bike for his birthday .".split(" "),
        ...
        # Test add-eos.
        samples = read_samples_file("test/data/samples-empty-3.txt", 3, add_eos=True)
